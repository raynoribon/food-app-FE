...
export class OrderService {
  ...
  getAllDistinctOrders(): Observable<any> {
    return this.http.get(this.server_url + '/orders', { headers: { Authorization: `Bearer ${this.custAuthService.getToken()}` } }).pipe(
      catchError((error) => {
        let errorMsg: string;
  ...
  getLatestOrderById(customerID: Number) {
    return this.http
      .get<ProductResponseModel[]>(
        this.server_url + '/orders/' + customerID
      )
      .pipe(
        catchError((error) => {
          let errorMsg: string;
  ...
  updateOrder(updateInfo: any): Observable<any> {
    ...
    return this.http
      .post(this.server_url + '/orders', updateInfo, { headers: headers })
      .pipe(
        catchError((error) => {
          let errorMsg: string;
          if (error.error instanceof ErrorEvent) {
